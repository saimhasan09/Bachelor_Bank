...
class TransferView(FormView):
    ...
    success_url = reverse_lazy(
        "loan_list"
    )  # Replace 'success' with your success URL name
    ...
    def form_valid(self, form):
        ...
        if amount > self.request.user.account.balance:
            messages.error(
                self.request, f"Transfer amount is greater than available balance"
            )
            return redirect("transfer")
        user_account.balance += amount
        self.request.user.account.balance -= amount
        messages.success(self.request, f"Transfer Done")
